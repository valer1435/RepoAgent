...
class TaskManager:
    """
    Manages a collection of tasks with dependencies and tracks their status."""
    ...
    def add_task(self, dependency_task_id: List[int], extra=None) -> int:
        """
        Registers a new task with its dependencies and associated data. Each task is assigned a unique ID, enabling tracking and management of relationships between tasks.
    ...
    def get_next_task(self, process_id: int):
        """
        Assigns a ready task to the requesting process, marking it as in-progress and updating its status. Returns the task details and ID, or (None, -1) if no tasks are currently available.

            This method checks for tasks that are ready to be processed (i.e., have no
            remaining dependencies and are in a 'not started' status). It updates the
            task's status to 'in progress' upon retrieval.
            ...
            Returns:
                tuple: A tuple containing the next available task object and its ID.
                       If no tasks are ready, returns (None, -1).
        """
