...
class ChatEngine:
    ...
    def __init__(self, project_manager):
        """
        Initializes the chat engine with settings for interacting with a language model, including API key, base URL, timeout, model name, and temperature.

            Args:
                project_manager: The project manager instance (not used in this method but part of class definition).
    ...
    def build_prompt(self, doc_item: DocItem, main_idea="", context_length=20000):
        """
        Constructs a prompt containing code details, its context within the project through references and callers, and existing documentation to facilitate docstring enhancement or generation. The prompt incorporates information about the code's type, name, content, and relationships with other components in the codebase.

            Args:
                doc_item: A DocItem object containing information about the code to be documented.
                main_idea: An optional user-defined main idea of the project, which can be used to enhance the existing docstring.
    ...
    def generate_doc(self, doc_item: DocItem):
        """
        Generates documentation for a given code element by querying a language model, incorporating project-level context when available. The generated output is then formatted for clarity.
    ...
    def generate_idea(self, list_items: str):
        """
        Generates a textual response based on provided components, utilizing the configured language settings. The method interacts with a language model to produce this output and logs token usage for monitoring purposes.

            This method uses an LLM to create an idea from the provided components,
            formatted with project language settings. It logs token usage and handles
            potential errors during the LLM call.

            Args:
                list_items: A string containing the list of items to base the idea on.

            Returns:
                str: The generated idea as a string, or raises an exception if an error occurs.
        """
    ...
    def summarize_module(self, module_desc: str):
        """
        Generates a concise summary of the provided module description, leveraging project settings for context and utilizing a language model to produce the output. Includes logging of token usage during the LLM interaction.

            This method leverages the LLM to generate a summary of the provided module
            description, incorporating project-level settings such as the main idea and
            programming language. It also logs token usage from the LLM call for debugging.

            Args:
                module_desc: The description of the module to summarize.

            Returns:
                str: The generated summary from the LLM.  Raises an exception if the LLM chat call fails.
        """
