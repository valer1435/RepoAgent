...
class DocItemType(Enum):
    ...
    Represents a documentation item within a repository.
        ...
        This class encapsulates information about different elements like classes,
        functions, directories, and variables found during documentation analysis.
        It provides methods for string representation, colored printing, and
        determining relationships between items.

        Attributes:
            _repo: The repository the item belongs to.
            _dir: The directory containing the item.
            _file: The file where the item is defined.
            _class: Information about a class (if applicable).
            _class_function: Information about a function within a class (if applicable).
            _function: Information about a standalone function (if applicable).
            _sub_function: Information about a nested function (if applicable).
            _global_var: Information about a global variable (if applicable).

        Methods:
            to_str(): Returns a string representation of the DocItemType.
            print_self(): Formats the node name with a color based on its type.
    ...
    def to_str(self):
        """
        Returns a descriptive string indicating the type of documentation item. Specifically identifies class, function, or directory items, otherwise returns the item's name.
            ...
            Returns:
                str: A string representing the type of DocItem, such as 'ClassDef',
                     'FunctionDef', or 'Dir'.  If the type is not recognized, returns
    ...
    def print_self(self):
        """
        Formats the item name with color-coding to visually distinguish between directories, files, classes, and functions.

            Args:
                from_node: The node whose type determines the color.

            Returns:
                str: A colored string representing the node's name.
        """
...
class DocItem:
    """
    Represents an item within a documentation tree.

    This class stores information about code elements, their relationships, and
    associated documentation content. It provides methods for traversing the tree,
    checking relationships between items, and managing task status."""
    ...
    def has_ans_relation(now_a: DocItem, now_b: DocItem):
        """
        Determines if a hierarchical relationship exists between two documentation items.
    ...
    def check_depth(self):
        """
        Determines the depth of this documentation item within the nested structure.
    ...
    def parse_tree_path(self, now_path):
        """
        Recursively constructs the path from the root to each documentation item within the repository's structure.
    ...
    def get_file_name(self):
        """
        Returns the module's file name, ensuring it retains the '.py' extension.
    ...
    def get_full_name(self, strict=False):
        """
        Returns the complete hierarchical name of the object, constructing it by traversing up its parent relationships. The resulting name is a path-like string with components separated by forward slashes. If duplicate names are encountered along the hierarchy, a version identifier is appended to ensure uniqueness.
    ...
    def find(self, recursive_file_path: list) -> Optional[DocItem]:
        """
        Locates a specific item within the repository’s structure by traversing its directory hierarchy.
    ...
    def check_has_task(now_item: DocItem, ignore_list: List[str] = []):
        """
        Recursively determines if a documentation item or any of its descendants require processing.

            Recursively checks if the current item or any of its children need to be
            generated based on the `need_to_generate` function and updates the
            `has_task` attribute accordingly.
    ...
    def print_recursive(
        ...
        """
        Recursively displays the item and its children, indicating their type and name, with indentation to represent hierarchy. Optionally shows diff status if changes are detected.
    ...
    Finds all references to a variable within a repository.
        ...
        Returns:
            list[tuple[str, int, int]]: A list of tuples, where each tuple contains the relative path to the referencing file,
                                         the line number of the reference, and the column number of the reference.  Excludes
                                         the original definition location. Returns an empty list if any error occurs during processing.
    """
...
class MetaInfo:
    """
    MetaInfo class for managing and analyzing repository documentation.

    This class provides functionalities to load, process, and save metadata
    about a software project's files, their relationships, and tasks related to
    documentation generation. It facilitates understanding the repository structure
    and creating hierarchical representations of its contents."""
    ...
    def init_meta_info(file_path_reflections, jump_files) -> MetaInfo:
        """
        Constructs a `MetaInfo` object representing the project's structure by analyzing the repository and identified files. It captures essential metadata for subsequent documentation processing.
    ...
    def from_checkpoint_path(
        ...
        """
        Loads MetaInfo from a checkpoint directory."""
    ...
    def checkpoint(self, target_dir_path: str | Path, flash_reference_relation=False):
        """
        Persists the project’s metadata and hierarchical structure to disk, ensuring data is saved for later use or recovery. This includes key information about the project and its organization.
    ...
    def print_task_list(self, task_dict: Dict[Task]):
        """
        Displays a table summarizing task details, including ID, reason for documentation generation, file path, and dependencies. Dependency information is truncated for brevity if excessively long.
    ...
    def get_all_files(self, count_repo=False) -> List[DocItem]:
        """
        Returns a list of all items (files, directories, and the repository root) within the project structure.
            ...
            Returns:
                A list of DocItem objects representing all files, directories, and optionally the root repo item.
        """
    ...
    def find_obj_with_lineno(self, file_node: DocItem, start_line_num) -> DocItem:
        """
        Locates the relevant code element encompassing a specific line number within the file structure.

            This method traverses the document tree starting from a given node,
            searching for the deepest child node whose code span includes the specified line number.

            Args:
                e_node: The starting DocItem node to begin the search from.
                start_line_num: The line number to locate within an object's code span.

            Returns:
                DocItem: The DocItem object containing the given line number, or the
                         current node if no qualifying child is found.
        """
    ...
    def parse_reference(self):
        """
        Analyzes and establishes relationships between code elements by identifying references within the repository, while respecting specified file and object whitelists and handling cases involving temporary or untracked files.

            This method analyzes files to identify references from one object to another,
            handling cases with jump files, fake files, and white lists. It updates
            the `reference_who` and `who_reference_me` attributes of DocItem objects
    ...
    def get_task_manager(self, now_node: DocItem, task_available_func) -> TaskManager:
        """
        Creates a task dependency graph from the repository’s hierarchical structure, assigning unique IDs to each documented item and managing dependencies based on references and children.

            Args:
                target_repo_hierarchical_tree: The root DocItem representing the repository hierarchy.
    ...
    def get_topology(self, task_available_func) -> TaskManager:
        """
        Integrates documentation details from a previous version of the repository's metadata.
    ...
    def to_hierarchy_json(self, flash_reference_relation=False):
        """
        Transforms a repository’s file structure and content into a JSON format, representing relationships between files and their metadata. This allows for structured access to information about the project's organization and contents.
    ...
    def from_project_hierarchy_json(
        ...
        """
        Constructs a hierarchical documentation structure by recursively processing project files, identifying code elements, and establishing relationships between them. It handles potential naming conflicts and refines item types based on content analysis.
            ...
            Args:
                ...
                repo_structure (dict): A dictionary representing the repository structure.
                target_meta_info: An object to store target meta information.
