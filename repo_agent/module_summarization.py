...
def summarize_repository(root_dir: str, repo_structure, chat_engine) ->Dict[
    str, Any]:
    """
    Summarizes the repository by generating summaries for each directory and its subdirectories.
    
    This function recursively traverses the directory structure of a given repository, identifies Python files, and generates summaries based on the provided repository structure and chat engine. It also handles ignored folders as specified in the settings.
    ...
    def summarize_directory(directory: Path, repo_structure, chat_engine,
        root_dir) ->Dict[str, Any]:
        """
    Summarizes a directory by extracting summaries of Python files and subdirectories.
    ...
    """
        ...
        for file_path in file_paths:
            if file_path.is_file() and file_path.suffix == '.py':
                ...
                if stripped_file_path in repo_structure and repo_structure[
                    stripped_file_path]:
                    for obj in list(filter(lambda x: x['type'] in [
                        'ClassDef', 'FunctionDef'], repo_structure[
                        stripped_file_path])):
                        if obj['md_content']:
        ...
        submodule_summaries = list(filter(lambda x: x['module_summary'], [
            summarize_directory(subdir, repo_structure, chat_engine,
            root_dir) for subdir in subdir_paths]))
        submodules_summaries_text = [submod['module_summary'] for submod in
            submodule_summaries]
        if not file_summaries and not submodules_summaries_text:
            module_summary = ''
        else:
            module_summary = create_module_summary(directory.name,
                file_summaries, submodules_summaries_text, chat_engine)
        return {'name': directory.name, 'path': str(directory),
            'file_summaries': file_summaries, 'submodules':
            submodule_summaries, 'module_summary': module_summary}
    root_path = Path(root_dir)
    return summarize_directory(root_path, repo_structure, chat_engine, root_dir
        )


def create_module_summary(name: str, file_summaries: List[str],
    submodule_summaries: List[str], chat_engine) ->str:
    """
    Creates a summary for a module based on file and submodule summaries.
    
    This method constructs a module summary by combining the name of the module, summaries of its files, and summaries of its submodules. It then uses a `ChatEngine` instance to generate a final summarized content. The `repo_agent` project is designed to automate the generation and management of documentation for a Git repository, ensuring that the documentation remains up-to-date and accurately reflects the current state of the codebase.
