...
class ChangeDetector:
    ...
    This class identifies modified and added Python files, analyzes their diffs,
    and extracts information about structural changes within the code. It also
    handles staging of files based on project-specific rules."""
        ...
        Returns a dictionary where keys are paths to staged Python files and values indicate whether each file is new (added) or modified.
            ...
            Returns:
                dict: A dictionary where keys are staged .py file paths and values
                      are boolean indicating if it's a new file.
        """
    ...
    def get_file_diff(self, file_path, is_new_file):
        """
        Retrieves the differences for a specified file, staging it if newly added. Returns these differences as lines of text.
    ...
    def parse_diffs(self, diffs):
        """
        Extracts added and removed lines with their corresponding line numbers from a diff string. It identifies changes based on '+' (added) and '-' (removed) prefixes, accounting for context lines indicated by '@@' markers to maintain accurate line number tracking.
    ...
    def identify_changes_in_structure(self, changed_lines, structures):
        """
        Detects which code structures are affected by changes in specific lines of code, categorizing them as added or removed.
    ...
    def get_to_be_staged_files(self):
        """
        Identifies files, including untracked and unstaged markdown documentation and project hierarchy files, that should be added to the staging area based on their relation to corresponding Python files or specific project settings. It prioritizes files within designated documentation directories and considers those linked to existing staged Python code.
    ...
    def add_unstaged_files(self):
        """
        Stages files that meet specified criteria using `git add`. Returns the list of staged file paths.
