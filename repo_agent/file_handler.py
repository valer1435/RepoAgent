...
class FileHandler:
    ...
    def __init__(self, repo_path, file_path):
        """
        Initializes a FileHandler with the repository and file paths, retrieving project settings to determine the target project hierarchy.
    ...
    def get_obj_code_info(
        ...
        """
        Collects details about a code element, including its type, name, parameters, location within the file, content, and whether it includes a return statement. This information is structured into a dictionary for further processing.
            ...
            Returns:
                dict: A dictionary containing information about the code object, including its type, name,
                      ...
                      and the column number where the name appears on the start line.
        """
    ...
    def get_modified_file_versions(self):
        """
        Retrieves the content of the current and immediately preceding versions of a file.
            ...
            Returns:
                tuple: A tuple containing the current version and previous version of the file as strings.
                       If the file has no prior history, the previous version will be None.
        """
    ...
    def get_end_lineno(self, node):
        """
        Determines the last line number spanned by a node and its descendants within the abstract syntax tree.
    ...
    def add_parent_references(self, node, parent=None):
        """
        Recursively sets the `parent` attribute of each child node to the current node.
    ...
    def get_functions_and_classes(self, code_content):
        """
        Identifies and extracts function and class definitions from code, including their names, line numbers, parameters, and docstrings.
    ...
    def generate_file_structure(self, file_path):
        """
        Creates a structured representation of a file, detailing its contents as either a directory or a collection of code elements like functions and classes with associated metadata.
            ...
            Returns:
                list: A list of dictionaries, where each dictionary represents either a directory or a code object (function/class)
                      ...
                      If it's a file, returns a list of dictionaries detailing functions and classes found within the file.
        """
    ...
    def generate_overall_structure(self, file_path_reflections, jump_files) -> dict:
        """
        Analyzes the repository to construct a structured representation of its files and folders, respecting `.gitignore` rules and skipping specific files based on defined criteria. It recursively generates detailed structures for each non-ignored file within the repository.
    ...
    def convert_to_markdown_file(self, file_path=None):
        """
        Generates a markdown representation of the code structure based on the project hierarchy data. It iterates through code objects, determining their nesting level and formatting them accordingly with headings and content.

            Args:
                file_path: The path to the file within the project hierarchy.
                    If None, uses the self.file_path attribute.

            Returns:
                str: A markdown formatted string representing the code structure
                     defined in the project hierarchy JSON.
        """
